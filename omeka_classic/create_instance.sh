...
sed -i "s/PASSWORD/${db_password}/g" /var/www/${db_name}/config/db.ini

echo "DONE WITH OMEKA CLASSIC DIRECTORIES!"
...
docker run --name ${db_name}-${ram} -dit --net=host --memory="${ram}m" --cpus="${cpu}" -v /var/www/${db_name}:/var/www/html/volume -v /run/mysqld/mysqld.sock:/run/mysqld/mysqld.sock sephirothkod/omeka-classic:${version} ${port}
